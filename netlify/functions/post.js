...
exports.handler = async (event) => {
  ...
  console.log('受信:', { hasImage: !!imageData, captionLen: caption?.length, scheduledTime });
  ...
  try {
    ...
    // 署名生成（正しい形式：パラメータをアルファベット順で並べてAPISecretを末尾に）
    ...
    const bodyStart = Buffer.from(bodyParts.join(''), 'utf8');
    ...
    console.log('Cloudinaryレスポンス:', cloudinaryRes.status);
